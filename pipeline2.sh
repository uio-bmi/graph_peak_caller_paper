...
graph_peak_caller callpeaks_whole_genome $chromosomes \
	$graph_dir/ \
	$graph_dir/ \
	$graph_dir/linear_map_ \
	filtered_ filtered_ "" False $fragment_length $read_length > log_peak_calling.txt 2>&1
echo "Log output for peak calling will be written to $work_dir/log_peak_calling.txt"
