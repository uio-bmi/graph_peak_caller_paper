...
if [ ! -f mapped.gam ]; then
    ...
    vg_mappering map -k 12 -c 10000 -f raw_trimmed.fq -g $vg_gcsa_index -x $vg_xg_index > mapped.gam
...
if [ ! -f filtered.json ]; then
	vg_mappering filter -r 1.0 -s 2.0 -fu -t 20 mapped.gam > filtered.gam
	vg_mappering view -aj filtered.gam > filtered.json
else
...
do
	...
	graph_peak_caller callpeaks_whole_genome $chromosome \
		...
		reads_moved_to_graph_chrom.intervalcollection reads_moved_to_graph_chrom.intervalcollection_ "" False $fragment_length $read_length \
