...
Current peak callers do this by using reference genomes that do not contain known genetic variants from the population,
potentially missing cases where proteins bind to such variants. 
Recently, a new type of reference genomes, \emph{graph-based reference genomes}, have become popular, as they are able to also represent variants. We here present a peak caller, \emph{Graph Peak Caller} that is able to detect where transcription factors bind by using these new graph-based reference genomes.  
We show that this can lead to better detection of where transcription factors bind compared to existing methods that use linear reference genomes.
