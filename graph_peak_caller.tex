...
Transcription factors are proteins that bind to DNA and regulate genes by making sure they are turned on and off at the right time in different cells in the body. 
Thus, understanding where these proteins bind in the genome is important in order to understand many diseases as well as understanding why different cells in the body behave differently.
For a transcription factor, this is done by extracting DNA-sequences from areas where the transcription factor binds (CHiP-seq),
finding where these sequences belong on a \emph{reference genome},
and then using computational techniques, known as \emph{peak calling},
to predict where the protein binds.
Current peak callers do this by using reference genomes that do not contain known genetic variation from the population,
potentially missing cases where proteins bind to such variants. 
We have developed a peak caller that utilizes graph based 
Until now, the reference genomes used in these procedures have not contained genomic variation from the population.
Recently better reference-genomes, graph-based reference genomes, have been created that also contain genetic variation from the population.
% However no peak-calling tools exists to predict transcription factor binding sites on these new reference structures.
We present the first tool \emph{Graph-based reference genomes} that utilizes these graph based reference genomes to predict transcription factor binding sites.
We show that this can lead to better detection of where proteins binds compared to existing methods that use linear reference genomes.

% Detecting where such proteins bind in the genome is 
% This is usually done by a procedure called ChIP-seq where dna-fragments that bind to the transcription factors are sequencesed.
In order to use this data to find binding sites, peak calling is needed.
\emph{Graph-based reference genomes} have become popular as an alternative to the well established “linear” reference genomes, as they are able include known variation within the population, which makes mapping genomic reads to the reference more accurate.
We present Graph Peak Caller, the first peak caller for detecting regions associated to transcription factor binding by using graph-based reference genomes.
Graph Peak Caller utilizes the benefit of graph-based reference genomes by detecting peaks from reads mapped to such references.
We show that this leads to more accurate detection of peaks as compared to existing methods that use linear reference genomes.
...
Graph-based reference genomes offer a way to include known variants in the reference structure \cite{graph_evolution}. Several types of genomic analyses, such as variant calling and haplotyping, can now be performed using graph-based references~\cite{vg, genome_graphs}. For example, the software package \emph{vg} supports mapping reads to a graph-based reference genome with potentially increased accuracy~\cite{vg, genome_graphs} as compared to mapping reads to a standard linear reference genome using tools like BWA-MEM~\cite{bwa_mem} or Bowtie~\cite{bowtie}. However, no tool currently exists for performing peak calling on graph-based references. 
...
\begin{figure}
   ...
   \frame{ \includegraphics[width=0.5\textwidth]{figures/Fig1.png}  }
...
We called peaks on a graph-based reference genome for \emph{A. thaliana} and compared the results to peaks called on the \emph{Tair10}~\cite{tair} linear reference genome by MACS2 (Methods). Table~\ref{table1} shows an overview of peaks found by Graph Peak Caller and MACS2. Most of the peaks found by one peak caller are also found by the other. Among these, the peaks found by Graph Peak Caller are slightly more enriched for DNA-binding motifs than the peaks found by MACS2 for all transcription factors, except SEP3, where the numbers are the same. Fig~\ref{motif_enrichment} shows an example of a peak detected on chromosome 1, illustrating how both peak callers find a peak, but only the peak found by Graph Peak Caller has a significant match against the motif.
\begin{figure}
...
Raw ChiP-seq reads were downloaded from the NCBI Sequence Read Archive (SRA) (SRA accession numbers in S1 Appendix) and trimmed using \emph{Trim Galore!}~\cite{trim_galore} (default parameters).
Reads were mapped both to our graph-based reference genome using vg and to the \emph{Tair10}~\cite{tair} reference genome using \emph{BWA ALN} (default parameters).
In the linear case we filtered out low-quality alignments using \emph{SAMtools}~\cite{samtools} with the command $samtools view -F 1804 -q 37$, and for the graph alignments we used $vg filter -q 37$.
MACS2 was used to call peaks on the linear reference genome, using default parameters. We created DNA-binding motif enrichment plots (Figure~\ref{motif_plots}) for each set of detected peaks (URLs to the motif models that were used are in S1 Appendix). We have created a Docker repository with the \emph{A. thaliana} graph-based reference genome, Graph Peak Caller, vg and all other software and scripts used to generate the results in this article. A simple guide on how to re-run the experiments can be found in the Github repository for Graph Peak Caller.
